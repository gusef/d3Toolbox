...
#'
#' @export
d3Toolbox <- function(message, width = NULL, height = NULL, elementId = NULL) {
...
#' Shiny bindings for d3Toolbox
#'
#' Output and render functions for using d3Toolbox within Shiny
...
#' @param expr An expression that generates a d3Toolbox
#' @param env The environment in which to evaluate \code{expr}.
#' @param quoted Is \code{expr} a quoted expression (with \code{quote()})? This
#'   is useful if you want to save an expression in a variable.
#'
#' @name d3Toolbox-shiny
#'
#' @export
d3ToolboxOutput <- function(outputId, width = '100%', height = '400px'){
  htmlwidgets::shinyWidgetOutput(outputId, 'd3Toolbox', width, height, package = 'd3Toolbox')
}

#' @rdname d3Toolbox-shiny
#' @export
renderD3Toolbox <- function(expr, env = parent.frame(), quoted = FALSE) {
  if (!quoted) { expr <- substitute(expr) } # force quoted
  htmlwidgets::shinyRenderWidget(expr, d3ToolboxOutput, env, quoted = TRUE)
}
...
D3ScatterOutput <- function(outputId, width = '100%', height = '400px'){
    htmlwidgets::shinyWidgetOutput(outputId, 'D3Scatter', width, height, package = 'D3Scatter')
}
